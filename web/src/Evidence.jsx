    ...
    <>
      ...
      */}
      ...
      <Panel>
        <table className="w-full table-fixed border-separate border-spacing-0">
          ...
          <tbody>
            ...
            <tr data-held-out="" className="bg-n-25">
              <td className={`${TD} border-l-2 border-l-warn pl-[10px] text-left`}>
                <span className="font-semibold text-n-900">
                  {heldOut.label}
                </span>
                <span
                  data-held-out-status=""
                  className="ml-2 rounded bg-warn/10 px-1.5 py-0.5 text-[10.5px] font-semibold uppercase tracking-[0.04em] text-warn"
                >
                  {heldOut.status}
                </span>
              </td>
              {Object.keys(heldOut.metrics).map((key) => (
                <td
                  key={key}
                  data-held-out-cell=""
                  className={`${TD} tnum text-right text-n-300`}
                >
                  —
                </td>
              ))}
      ...
      <p className="mt-2 text-body-sm text-n-500">
        Seed 99 has never been scored: no metric in this row has been computed
        or read, and nothing in the engine was tuned against it. It is generated
        by the
        same frozen generator and sits on disk untouched; the row above is
        waiting for one run, once.
      </p>
    </>
  )
}
